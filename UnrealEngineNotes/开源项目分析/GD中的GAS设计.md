...
![[开源项目分析/GD中的GAS设计/6.png]]

这里有个显示伤害数值和给照成伤害的人赏金的操作，给赏金是动态生成ge然后应用上去的。
![[开源项目分析/GD中的GAS设计/7.png]]
...
![[开源项目分析/GD中的GAS设计/8.png]]

这在HUD中使用到，这也是gd文档中8.2中提到的点
![[开源项目分析/GD中的GAS设计/9.png]]
...
![[开源项目分析/GD中的GAS设计/10.png]]

冷却开始的回调
![[开源项目分析/GD中的GAS设计/11.png]]

冷却tag被移除的时候触发的回调
下面的函数和action rpg中差不多
![[开源项目分析/GD中的GAS设计/12.png]]

蓝图中的使用，冷却开始的时候，剩余时间小于0说明是本地预测的，大于0是服务器开始冷却的时刻
![[开源项目分析/GD中的GAS设计/13.png]]

这是代码中写死的
![[开源项目分析/GD中的GAS设计/14.png]]
...
![[开源项目分析/GD中的GAS设计/29.png]]

给ga加上默认阻挡标签，死亡和眩晕。
![[开源项目分析/GD中的GAS设计/30.png]]

火枪GA这里AbilityTag和ActivationOwnedTag用的是同一个tag
blocktag 是加了所有技能
![[开源项目分析/GD中的GAS设计/31.png]]

这里通过匹配asc上的标签判断是否使用下蹲的动画。
![[开源项目分析/GD中的GAS设计/32.png]]

C++中构造AT要手动激活
蓝图中会被编辑器自动激活
![[开源项目分析/GD中的GAS设计/33.png]]

这里有一处和我们平时使用不同的地方
PlayMontageAndWaitForEvent的参数传入的FGameplayTagContainer是空构造
![[开源项目分析/GD中的GAS设计/34.png]]
...
![[开源项目分析/GD中的GAS设计/35.png]]

在角色在服务器被占有的时候初始化能力
![[开源项目分析/GD中的GAS设计/36.png]]

在赋予能力的时候绑定输入
![[开源项目分析/GD中的GAS设计/37.png]]

# 监听Tag的变化
在监听冷却改变的蓝图异步任务中
监听冷却改变
![[开源项目分析/GD中的GAS设计/38.png]]

小兵初始化的时候监听眩晕的标签
![[开源项目分析/GD中的GAS设计/39.png]]

触发后会取消某些能力
![[开源项目分析/GD中的GAS设计/40.png]]

PlayerState初始化的时候也是监听眩晕的标签
![[开源项目分析/GD中的GAS设计/41.png]]

同样触发的时候取消些能力
![[开源项目分析/GD中的GAS设计/42.png]]

# GE资源Tag
有的ge会有ge的资源tag，就是在死亡的时候移除这个ge
![[开源项目分析/GD中的GAS设计/43.png]]

GD角色类中有这样一段代码
用RemoveActiveEffectsWithTags这个接口
![[开源项目分析/GD中的GAS设计/44.png]]

asset tag也也会被纳入考虑
![[开源项目分析/GD中的GAS设计/45.png]]

# C++中监听属性改变

![[开源项目分析/GD中的GAS设计/46.png]]

![[开源项目分析/GD中的GAS设计/47.png]]

# CharacterMovementComponent
在GD的CMC中定义了自定义的FSavedMove_Character
![[开源项目分析/GD中的GAS设计/48.png]]

和自定义的FNetworkPredictionData_Client_Character
![[开源项目分析/GD中的GAS设计/49.png]]

GD CMC中在服务器一直更新flag
需要看这篇文章[https://zhuanlan.zhihu.com/p/694601656](https://zhuanlan.zhihu.com/p/694601656)
![[开源项目分析/GD中的GAS设计/50.png]]

savemove的一些操作
清理和比对标识
![[开源项目分析/GD中的GAS设计/51.png]]

判断是否需要合并savemove
![[开源项目分析/GD中的GAS设计/52.png]]

重写FGDNetworkPredictionData_Client::AllocateNewMove为返回我们自定义的savedmove
![[开源项目分析/GD中的GAS设计/53.png]]

# 处理Scoped Prediction Key 无效的情况

sprint的GA使用了wait net sync节点
![[开源项目分析/GD中的GAS设计/55.png]]

主要为了解决wait delay后Scoped Prediction Key 无效的情况
![[开源项目分析/GD中的GAS设计/54.png]]

看了sprint的ga中应用的ge是没有更改移动的属性的，主要是加sprint的tag
主要都是在移动组件中做修改
![[开源项目分析/GD中的GAS设计/56.png]]